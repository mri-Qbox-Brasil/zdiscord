...
const BotStatusMessages = ["{servername}", "{playercount} online"];
...
const LoggingWebhooks = {
    example:
        "https://discord.com/api/webhooks/000000000/sEcRRet-ToK3n_5tUfF_tH8t_YUo-S40u1d-n07-sHar3",
};
...
module.exports = {
    ...
    NameChanger: {
        ...
        ShowCitizenID: getConBool(
            "discord_show_citizen_id",
            NameChanger.ShowCitizenID
        ),
    },
    BoosterReward: {
        Enabled: getConBool("discord_enable_booster_reward", BoosterReward.Enabled),
        BoosterRoleId: GetConvar(
            "discord_booster_role_id",
            BoosterReward.BoosterRoleId
        ),
        RewardServerTrigger: GetConvar(
            "discord_booster_reward_trigger",
            BoosterReward.RewardServerTrigger
        ),
        ExpiryRewardServerTrigger: GetConvar(
            "discord_booster_expiry_trigger",
            BoosterReward.ExpiryRewardServerTrigger),
    },
...
/** returns int or default int provided
 * @param {string} con - string of comma separated values
 * @param {string|Array} def - string of comma separated values
 * @returns {object} - array of discord ids */
function getConInt(con, def) {
    const ret = GetConvar(con, def);
    if (typeof ret == "string") return parseInt(ret);
    if (Array.isArray(ret)) return ret;
    if (!ret) return 0;
    return parseInt(ret);
}
